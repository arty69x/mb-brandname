...
function Brand() {
  return (
    <Link href='/' className='leading-none'>
      <p className='text-[28px] font-light uppercase tracking-[-0.05em] lg:text-[34px]'>m3</p>
      <p className='-mt-1 text-[10px] tracking-[0.01em]'>MB BRANDNAME</p>
    </Link>
  );
}
    ...
    >
      <div className='container mx-auto flex items-center justify-between px-4 py-4 lg:py-5'>
        ...
        <nav className='hidden items-center gap-8 lg:flex'>
          {links.map((l) => (
            <Link key={l.href} className='text-[13px] font-semibold tracking-[0.16em] uppercase' href={l.href}>
              {l.label}
            </Link>
          ))}
        </nav>
