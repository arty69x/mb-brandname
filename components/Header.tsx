    ...
    <header className="border-b border-black/10 bg-white">
      <div className="hidden h-[72px] items-center justify-between px-4 md:px-6 lg:flex lg:px-8">
        <div className="flex items-center gap-4">
          ...
          <span className="text-xs uppercase tracking-[0.24em]">MBbrandname</span>
        </div>
        <nav className="flex items-center gap-8">
          {navLinks.map((link) => (
            <Link key={link.href} href={link.href} className="text-sm transition-all duration-300 ease-in-out hover:opacity-60">
              {link.label}
            </Link>
          ))}
        </nav>
        <div className="flex items-center gap-4">
          ...
          <Link href="/cart" aria-label="Cart" className="relative transition-all duration-300 ease-in-out hover:opacity-60">
            <ShoppingBag size={18} />
            {cartCount > 0 ? <span className="absolute -right-2 -top-2 h-4 w-4 rounded-full bg-black text-[10px] text-white">{cartCount}</span> : null}
          </Link>
        </div>
      </div>
      ...
      <div className="relative flex h-[56px] items-center justify-between px-4 lg:hidden">
        ...
        <Link href="/cart" aria-label="Cart" className="transition-all duration-300 ease-in-out hover:opacity-60">
          <ShoppingBag size={20} />
        </Link>
      </div>
    </header>
