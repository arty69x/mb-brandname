      ...
      <main>
        <section className='py-10 lg:py-14'>
          <div className='container mx-auto px-4'>
            ...
            {featured ? (
              <article className='grid gap-6 overflow-hidden rounded-2xl border border-[#d9d9d9] bg-white lg:grid-cols-[1.05fr_0.95fr]'>
                <img src={featured.image} alt={featured.title} className='h-full min-h-[320px] w-full object-cover' />
                <div className='p-6 lg:p-8'>
                  <p className='text-[11px] uppercase tracking-[0.16em] text-[#8b8b8b]'>Featured article</p>
                  <h2 className='mt-3 text-[24px] font-light leading-[1.3]'>{featured.title}</h2>
                  <p className='mt-4 text-[14px] leading-[1.8] text-[#6b6b6b]'>{featured.excerpt}</p>
            ...
            <div className='mt-8 grid grid-cols-1 gap-8 md:grid-cols-2'>
              {rest.map((post) => (
                <Link key={post.slug} href={`/blog/${post.slug}`} className='overflow-hidden rounded-2xl border border-[#d9d9d9] bg-white p-4'>
                  ...
                  <p className='mt-2 text-[14px] text-[#6b6b6b]'>{post.excerpt}</p>
                </Link>
              ))}
            </div>
          </div>
